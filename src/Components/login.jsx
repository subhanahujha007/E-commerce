...
const login = () => {
  ...
  return (
    <div className="login">
        ...
        <motion.button initial={{skew:0}} animate={{skew:[0,25,-25,0],transition:{delay:0.6,duration:0.3}}} >LOG IN</motion.button>
        <motion.h3 initial={{y:700}} animate={{y:0,transition:{delay:0.9,duration:0.4}}}>New user?</motion.h3>
        <motion.button initial={{skew:0}} animate={{skew:[0,25,-25,0],transition:{delay:0.6,duration:0.3}}} ><Link to="/signup">SING UP</Link></motion.button>
        </motion.form>
    </div>
  )}
